...
export function DropDown({ width, height, textClassName, useBaseStyle=true, children, onChange }: DropDownProps) {
        ...
        >
          {React.Children.map(children, (child) => {
            if (React.isValidElement<DropDownOptionProps>(child))
              return React.cloneElement(child, { useBaseStyle: useBaseStyle, onSelect: handleSelect });
          })}
        </ul>
      );
  };
  ...
  return (
    <div className={`relative flex items-center cursor-pointer ${useBaseStyle ? baseStyle : ''}`} style={{ width: width, height: height ? height : 'auto' }}>
      <div
        ref={dropDownRef}
        className="w-full h-6 flex justify-between items-center md:h-[22px]"
        onClick={() => setIsopen(!isOpen)}
      >
        <span className={`${ useBaseStyle ? ( isOpen ? 'text-gray-50' : 'text-gray-200' ) : '' } ${textClassName}`}>{categoryName}</span>
        <div className="relative w-6 h-6 md:w-[22px] h-[22px]">
          <Image src={iconSrc} alt="dropIcon" fill sizes='(max-width: 767px) 24px, (min-width: 768px) 22px' />
        </div>
      </div>
      <ShowDropDownOptions />
    </div>
